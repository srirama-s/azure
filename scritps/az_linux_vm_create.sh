...
declare deploymentName="linuxvmdeploy"
...
templateFilePath="azure-linux-vm-vnet.json"
...
if [ $? != 0 ]; then
	...
	echo "Deleting existing resource group and recreating ..."
	set -e
	(
		set -x
		az group delete --name $resourceGroupName -y 1> /dev/null
		sleep 3
		az group create --name $resourceGroupName --location $resourceGroupLocation 1> /dev/null
